(function ($) {
  var halo = {
    ...
    initSlideshow: function () {
      ...
      var slickSlideshow = $(
        ".home-slideshow:not(.hasvideo) [data-init-slideshow]"
      );
      if (slickSlideshow.length) {
        ...
        slickSlideshow.each(function () {
          ...
          if (fade == true) {
            ...
            _self
              .parents(".init-slider")
              .find("[data-init-slideshow]")
              .css(
                "transform",
                "translate3d(-" +
                  w_slider *
                    (_self.find(".item.slide-active").data("index") - 1) +
                  "px, 0, 0)"
              );
          }
          if (autoplay) {
            let slickShow = _self.closest(".home-slideshow");
            halo.setTimer(slickShow, autoplaySpeed);
            // ===========================================
            // auto = setInterval(function () {
            //   action.find(".slick-next").trigger("click");
            // }, autoplaySpeed);
        ...
        $("body")
          .off("click.arrows", ".slide-action .slick-arrow")
          .on("click.arrows", ".slide-action .slick-arrow", function (e) {
            ...
            var autoplaySpeed = _parent
              .find("[data-init-slideshow]")
              .data("autoplay-speed");
            var autoplay = _parent
              .find("[data-init-slideshow]")
              .data("autoplay");
            var auto_playvideo = _parent
              .find("[data-init-slideshow]")
              .data("auto-video");
              ...
              if (index > 1) {
                ...
                if (fade == false) {
                  active
                    ...
                    .css(
                      "transform",
                      "translate3d(-" + w_slider * (index - 2) + "px, 0, 0)"
                    );
                } else {
              ...
              if (index < last_index) {
                ...
                if (fade == false) {
                  active
                    ...
                    .css(
                      "transform",
                      "translate3d(-" + w_slider * index + "px, 0, 0)"
                    );
                } else {
                  _parent.find(".slide-active").css("left", "0");
                }
            ...
            _parent
              .find('li[data-dot-index="' + newActive + '"]')
            ...
            if (autoplay) {
              // 重置计时器
              halo.clearTimer(_parent)
              halo.setTimer(_parent, autoplaySpeed)
              // ==========================================
        ...
        $("body")
          .off("click.dots", ".slide-action li")
          .on("click.dots", ".slide-action li", function (e) {
            var _parent = $(this).parents("[data-loader-script]");
            var active = _parent.find(".slide-active");
            ...
            if ($(this).hasClass("slick-active")) {
              ...
              if (fade == false) {
                active
                  ...
                  .css(
                    "transform",
                    "translate3d(-" + w_slider * (index - 1) + "px, 0, 0)"
                  );
              } else {
                _parent.find(".slide-active").css("left", "0");
              }
    ...
    clearTimer: function (slickShow) {
      let slideShowId = slickShow.attr("id")
      clearInterval(halo.auto[slideShowId]);
    },
    setTimer: function (slickShow, autoplaySpeed) {
      let nextArrow = slickShow.find(".slick-next");
      var timerId = setInterval(function () {
        nextArrow.trigger("click");
      }, autoplaySpeed);
      // 将定时器存储在 auto 对象中，以轮播元素的唯一标识作为属性名
      let slideShowId = slickShow.attr("id");
      halo.auto[slideShowId] = timerId;
    },
  };
  halo.initSlideshow();
  $(window).resize(function () {
    ...
    if (slickSlideshow.length) {
      ...
      slickSlideshow.each(function () {
        ...
        $(this).find(".item").css("width", w_slider);
        setTimeout(() => {
          h_slider = _self.find(".item.slide-active").outerHeight();
          if (fade == true) {
            _self.css({
              height: h_slider,
            });
  ...
  window.autoPlaySlideShowHalo = halo;
})(jQuery);
